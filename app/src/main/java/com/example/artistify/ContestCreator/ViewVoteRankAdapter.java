...
public class ViewVoteRankAdapter extends RecyclerView.Adapter<ViewVoteRankAdapter.ViewHolder> {
    ...
    public ViewVoteRankAdapter(Context context, List<ContestEntry> mImage) {
    ...
    @Override
    public void onBindViewHolder(@NonNull ViewVoteRankAdapter.ViewHolder holder, int position) {
...
//                            .placeholder(R.drawable.progress_animation_profile).centerCrop().into(holder.uploaded_image);
                ...
                for(DataSnapshot dataSnapshot:snapshot.getChildren()){
                    ...
                    if(contestEntry1!=null){
                        ...
                        if(cont_id.equals(contestEntry.getContestID())&&user_id.equals(contestEntry.getUserID())){ //change with contestEntry.getUserID() if not working well
                            contestEntryReference.child(contestEntryID).child("vote_rank").setValue(position+1);
                        }
                    }
                }
            }
        ...
        holder.uploaded_image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                dialog_entry.setContentView(R.layout.contest_entry_cc_layout_dialog);
                dialog_entry.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));

                TextView image_title_CC= dialog_entry.findViewById(R.id.image_title_selected_contest_creator);
                TextView entry_category_CC= dialog_entry.findViewById(R.id.image_category_contest_creator);
                TextView date_submit_CC= dialog_entry.findViewById(R.id.date_submit_photo_contest_creator);

                LinearLayout linear_manage_reject_entry=dialog_entry.findViewById(R.id.linear_manage_reject_entry);

                TextView actual_place_CC=dialog_entry.findViewById(R.id.actual_place_contest_creator);
                TextView points_CC=dialog_entry.findViewById(R.id.points_contest_creator);

                points_CC.setText(String.valueOf(contestEntry.getVote_count()));
                if(contestEntry.getVote_rank()!=1000000000){
                    actual_place_CC.setText(String.valueOf(contestEntry.getVote_rank()));
                }


                linear_manage_reject_entry.setVisibility(View.GONE);

                TextView participant_name_CC= dialog_entry.findViewById(R.id.participant_name_contest_creator);
                ...
                ImageView imageSelected_CC= dialog_entry.findViewById(R.id.imageViewSelected_contest_creator);
                ProgressBar myProgressbar=dialog_entry.findViewById(R.id.progress_image_loader);

                ImageView imageViewClose= dialog_entry.findViewById(R.id.imageViewClose_contest_creator);
...
//                                    .fitCenter().override(width_screen,height_screen).into(imageSelected_CC);
                            ...
                            Glide.with(context).load(url)
                                    .diskCacheStrategy(DiskCacheStrategy.NONE).fitCenter().override(width_screen,height_screen).dontAnimate()
                                    .listener(new RequestListener<Drawable>() {
                                        @Override
                                        public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                                            return false;
                ...
                imageViewClose.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        dialog_entry.dismiss();
                    }
                });
                dialog_entry.show();
                WindowManager.LayoutParams layoutParams = new WindowManager.LayoutParams();
                layoutParams.copyFrom(dialog_entry.getWindow().getAttributes());
                layoutParams.width = WindowManager.LayoutParams.MATCH_PARENT;
                layoutParams.height = WindowManager.LayoutParams.MATCH_PARENT;
                dialog_entry.getWindow().setAttributes(layoutParams);
            }
        });
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            dialog_entry = new Dialog(itemView.getContext());
